import{A as l,U as _,F as g,I as p,aK as S,aL as A,aM as I,aN as N,aO as E}from"./drawGraph-BDbP87ZH.js";import{Q as C}from"./quad-DP6llhdf.js";var L=`flat out int v_numParticles;
out vec2 v_uv;
void main() {
    v_numParticles = numParticles;
    v_uv = position;
    gl_Position = vec4(position, 1.0, 1.0);
}`,h=`#define FBM_OCTAVES 1
...
}`;function M({vertices:n,colorsTexture:e,depthSampleTex:t,lissajousUniforms:c,displayUniforms:o}){return{data:{draw:l(n,()=>L,()=>h,{uniforms:{lissajousUniforms:c,displayUniforms:o},textures:{colors:{texture:e,sampler:{filter:"nearest",wrap:"clamp"}},depth_src:{texture:t,sampler:{filter:"nearest",wrap:"clamp"}}}},{blendMode:"alpha"})}}}var b=`#define PI 3.14159265359
vec3 lissajous(
    float t,
    float angle,
    float a,
...
}`;const r=1e3;function R({props:n,quad:e,lissajousUniforms:t,displayUniforms:c}){const o=_({maxInstanceCount:r,attributes:{instanceId:"int"}},{count:r,instances:()=>{const s=[];for(let v=0;v<r;v++)s.push({instanceId:v});return s}},[]),a=g({numLineSegments:"int"},()=>({numLineSegments:r}),[n.time]),i=l(e,()=>b,()=>F,{uniforms:{curveUniforms:a,lissajousUniforms:t,displayUniforms:c},instances:o},{depthTest:!0,blendMode:"alpha"});return{lineSegmentInstances:o,curveUniforms:a,data:{draw:i}}}var B=`#define PI 3.14159265359
vec3 lissajous(
    float t,
    float angle,
    float a,
...
}`;const O=500;function U({props:n,quad:e,numberOfColors:t,colorsTexture:c,lissajousUniforms:o,displayUniforms:a}){const i=_({maxInstanceCount:O,attributes:{instanceId:"int"}},{count:t,instances:()=>{const v=[];for(let f=0;f<t;f++)v.push({instanceId:f});return v}},[]),s=l(e,()=>B,()=>T,{uniforms:{lissajousUniforms:o,displayUniforms:a},instances:i,textures:{colors:{texture:c,sampler:{filter:"nearest",wrap:"edge"}}}},{depthTest:!0,blendMode:"alpha"});return{pointInstances:i,data:{draw:s}}}const x=100;function D(n){const e=g({resolution:"vec2"},()=>({resolution:[n.general.canvasDimensions[0],n.general.canvasDimensions[1]]}),[n.general.canvasDimensions[0],n.general.canvasDimensions[1]]),t=g({lissajous_a:"vec2",lissajous_b:"vec2",lissajous_c:"vec2",numParticles:"int",numMaxParticles:"int",time:"float"},()=>({numParticles:n.colors.length,numMaxParticles:x,time:[n.time],lissajous_a:n.lissajousParams.a,lissajous_b:n.lissajousParams.b,lissajous_c:n.lissajousParams.c}),[n.colors.length,n.time,n.lissajousParams.a,n.lissajousParams.b,n.lissajousParams.c]),c=p({width:x,height:1},()=>{const a=[];for(let i=0;i<x;i++){const s=n.colors[i]||{r:0,g:0,b:0};a.push(s.r*255,s.g*255,s.b*255,255)}return a},[JSON.stringify(n.colors)]),o=C([{x:-1,y:-1,u:0,v:0},{x:1,y:-1,u:1,v:0},{x:1,y:1,u:1,v:1},{x:-1,y:1,u:0,v:1}],[]);return{display_uniforms:e,lissajous_uniforms:t,colors_texture:c,quad:o}}var j=`out vec2 v_uv;
...
}`;function q({props:n,vertices:e,lissajousUniforms:t,displayUniforms:c}){const o=l(e,()=>j,()=>G,{uniforms:{lissajousUniforms:t,displayUniforms:c},textures:{}},{blendMode:"alpha"});return{texture:p({width:n.general.canvasDimensions[0],height:n.general.canvasDimensions[1]},[o],[n.general.canvasDimensions[0]]),data:{draw:o}}}const u={colors:[{r:Math.pow(24/255,1.5),g:Math.pow(0,1.5),b:Math.pow(97/255,1.5)},{r:Math.pow(79/255,1.5),g:Math.pow(23/255,1.5),b:Math.pow(135/255,1.5)},{r:Math.pow(235/255,1.5),g:Math.pow(52/255,1.5),b:Math.pow(120/255,1.5)},{r:Math.pow(235/255,1.5),g:Math.pow(119/255,1.5),b:Math.pow(60/255,1.5)}],time:0,lissajousParams:{a:[1,.6],b:[2,.4],c:[1,1]},general:{canvasDimensions:[800,600]}};function k(n){const e=D(n),t=q({props:n,vertices:e.quad,displayUniforms:e.display_uniforms,lissajousUniforms:e.lissajous_uniforms}),c=M({vertices:e.quad,colorsTexture:e.colors_texture,displayUniforms:e.display_uniforms,lissajousUniforms:e.lissajous_uniforms,depthSampleTex:t.texture}),o=R({props:n,quad:e.quad,lissajousUniforms:e.lissajous_uniforms,displayUniforms:e.display_uniforms}),a=U({props:n,quad:e.quad,numberOfColors:n.colors.length,colorsTexture:e.colors_texture,lissajousUniforms:e.lissajous_uniforms,displayUniforms:e.display_uniforms}),i=p({width:n.general.canvasDimensions[0],height:n.general.canvasDimensions[1]},[c.data.draw,o.data.draw,a.data.draw],[n.general.canvasDimensions[0],n.general.canvasDimensions[1]]);return{shared_inputs:e,drawGradient:c,drawLissajousCurve:o,drawPoints:a,sampleDepthTex:t,outputTexture:i}}function V(n){const e={...u,time:n};return k(e)}const z=new S,Y=document.getElementById("mainCanvas"),P=Y.getContext("webgl2",{antialias:!1});if(!P)throw new Error("Unable to initialize WebGL2");const K=new I(P),y=new A(K);z.subscribe((n,e)=>{const t=Object.fromEntries(n);y.submit(e,t),y.draw("outputTexture")});const X=new N;function Z(n,e){const{addedAssets:t,deletedAssetIds:c,graphId:o}=X.update(V(n));e&&E(t[o]),z.transaction(t,c,o)}const Q=document.getElementById("inputRange");let m=parseFloat(Q.value)/100;const $=document.getElementById("saveFrameBtn");$.addEventListener("click",()=>{const n=y.capture("outputTexture"),e=u.general.canvasDimensions[0],t=u.general.canvasDimensions[1],c=document.createElement("canvas");c.width=e,c.height=t;const o=c.getContext("2d");if(!o)throw new Error("Unable to create 2D context for image saving");const a=o.createImageData(e,t);a.data.set(n),o.putImageData(a,0,0);const i=document.createElement("a");i.download="frame.png",i.href=c.toDataURL(),i.click()});let d=0;function w(){m=d%300/300,Z(m,d===0),d+=.5,requestAnimationFrame(w)}w();
