import{B as s,F as _,I as y,A as f,aK as S,aL as N,aM as A,aN as C,aO as E}from"./drawGraph-tqQ3UCYH.js";import{Q as b}from"./quad-CQRAswkw.js";var L=`flat out int v_numParticles;
out vec2 v_uv;
void main() {
    v_numParticles = numParticles;
    v_uv = position;
    gl_Position = vec4(position, 1.0, 1.0);
}`,R=`#define FBM_OCTAVES 1
...
}`;function h({vertices:n,colorsTexture:e,depthSampleTex:t,lissajousUniforms:c,displayUniforms:o}){return{data:{draw:s(n,()=>L,()=>R,{uniforms:{lissajousUniforms:c,displayUniforms:o},textures:{colors:{texture:e,sampler:{filter:"nearest",wrap:"clamp"}},depth_src:{texture:t,sampler:{filter:"nearest",wrap:"clamp"}}}},{blendMode:"alpha"})}}}var F=`#define PI 3.14159265359
vec3 lissajous(
    float t,
    float angle,
    float a,
...
}`;const l=1e4;function T({props:n,quad:e,lissajousUniforms:t,displayUniforms:c}){const o=_({maxInstanceCount:l,attributes:{instanceId:"int"}},{count:l,instances:()=>{const v=[];for(let r=0;r<l;r++)v.push({instanceId:r});return v}},[]),i=y({numLineSegments:"int"},()=>({numLineSegments:l}),[n.time]),a=s(e,()=>F,()=>M,{uniforms:{curveUniforms:i,lissajousUniforms:t,displayUniforms:c},instances:o},{depthTest:!0,blendMode:"alpha"});return{lineSegmentInstances:o,curveUniforms:i,data:{draw:a}}}var B=`#define PI 3.14159265359
vec3 lissajous(
    float t,
    float angle,
    float a,
...
}`;const O=500;function U({props:n,quad:e,numberOfColors:t,colorsTexture:c,lissajousUniforms:o,displayUniforms:i}){const a=_({maxInstanceCount:O,attributes:{instanceId:"int"}},{count:t,instances:()=>{const r=[];for(let x=0;x<t;x++)r.push({instanceId:x});return r}},[]),v=s(e,()=>B,()=>D,{uniforms:{lissajousUniforms:o,displayUniforms:i},instances:a,textures:{colors:{texture:c,sampler:{filter:"nearest",wrap:"edge"}}}},{depthTest:!0,blendMode:"alpha"});return{pointInstances:a,data:{draw:v}}}const g=100;function j(n){const e=y({resolution:"vec2"},()=>({resolution:[n.general.canvasDimensions[0],n.general.canvasDimensions[1]]}),[n.general.canvasDimensions[0],n.general.canvasDimensions[1]]),t=y({lissajous_a:"vec2",lissajous_b:"vec2",lissajous_c:"vec2",xyRotation:"vec2",numParticles:"int",numMaxParticles:"int",time:"float",mixingIntensity:"float",noiseIntensity:"float"},()=>({numParticles:n.colors.length,numMaxParticles:g,time:[n.time],lissajous_a:n.lissajousParams.a,lissajous_b:n.lissajousParams.b,lissajous_c:n.lissajousParams.c,mixingIntensity:n.mixingIntensity,xyRotation:n.xyRotation,noiseIntensity:n.noiseIntensity}),[n.colors.length,n.time,n.lissajousParams.a,n.lissajousParams.b,n.lissajousParams.c]),c=f({width:g,height:1},()=>{const i=[];for(let a=0;a<g;a++){const v=n.colors[a]||{r:0,g:0,b:0};i.push(v.r*255,v.g*255,v.b*255,255)}return i},[JSON.stringify(n.colors)]),o=b([{x:-1,y:-1,u:0,v:0},{x:1,y:-1,u:1,v:0},{x:1,y:1,u:1,v:1},{x:-1,y:1,u:0,v:1}],[]);return{display_uniforms:e,lissajous_uniforms:t,colors_texture:c,quad:o}}var Y=`out vec2 v_uv;
...
void main() {
    ...
    outColor = vec4(vec3(result),1.0);
}`;function q({props:n,vertices:e,lissajousUniforms:t,displayUniforms:c}){const o=s(e,()=>Y,()=>G,{uniforms:{lissajousUniforms:t,displayUniforms:c},textures:{}},{blendMode:"alpha"});return{texture:f({width:Math.floor(n.general.canvasDimensions[0]/4),height:Math.floor(n.general.canvasDimensions[1]/4),format:"rgba32f"},[o],[n.general.canvasDimensions[0]]),data:{draw:o}}}function p(n,e){return s(n,()=>`
          out vec2 v_uv;
          void main() {
            gl_Position = vec4(position, 0.0, 1.0);
            v_uv = vec2(u,v);
          ...
          void main() {
            ...
            outColor = color;
          }`,{uniforms:{},textures:{src:{sampler:{edge:"clamp",filter:"linear"},texture:e}}})}var X=`out vec2 v_uv;
...
void main() {
    ...
    outColor = col;
    float grainValue = grain(uv, resolution, time);
    outColor.rgb += (grainValue - 0.5) * noiseIntensity * 0.8;
}`;function k({quad:n,texture:e,lissajousUniforms:t,displayUniforms:c}){return{data:{draw:s(n,()=>X,()=>V,{uniforms:{lissajousUniforms:t,displayUniforms:c},textures:{src:{sampler:{edge:"clamp",filter:"linear"},texture:e}}})}}}const u={colors:[{r:Math.pow(24/255,1.5),g:Math.pow(0,1.5),b:Math.pow(97/255,1.5)},{r:Math.pow(79/255,1.5),g:Math.pow(23/255,1.5),b:Math.pow(135/255,1.5)},{r:Math.pow(235/255,1.5),g:Math.pow(52/255,1.5),b:Math.pow(120/255,1.5)},{r:Math.pow(235/255,1.5),g:Math.pow(119/255,1.5),b:Math.pow(60/255,1.5)}],time:0,lissajousParams:{a:[1,.6],b:[2,.4],c:[1,1]},mixingIntensity:.5,xyRotation:[0,0],noiseIntensity:.2,enableNoise:!0,general:{canvasDimensions:[800,600],showLissajousFigure:!0}};function K(n){const e=j(n),t=q({props:n,vertices:e.quad,displayUniforms:e.display_uniforms,lissajousUniforms:e.lissajous_uniforms}),c=h({vertices:e.quad,colorsTexture:e.colors_texture,displayUniforms:e.display_uniforms,lissajousUniforms:e.lissajous_uniforms,depthSampleTex:t.texture}),o=T({props:n,quad:e.quad,lissajousUniforms:e.lissajous_uniforms,displayUniforms:e.display_uniforms}),i=U({props:n,quad:e.quad,numberOfColors:n.colors.length,colorsTexture:e.colors_texture,lissajousUniforms:e.lissajous_uniforms,displayUniforms:e.display_uniforms}),a=f({width:n.general.canvasDimensions[0],height:n.general.canvasDimensions[1]},[c.data.draw],[n.general.canvasDimensions[0],n.general.canvasDimensions[1]]),v=k({quad:e.quad,texture:a,lissajousUniforms:e.lissajous_uniforms,displayUniforms:e.display_uniforms}),r=f({width:n.general.canvasDimensions[0],height:n.general.canvasDimensions[1]},[v.data.draw],[n.general.canvasDimensions[0],n.general.canvasDimensions[1]]),x=f({width:n.general.canvasDimensions[0],height:n.general.canvasDimensions[1]},n.general.showLissajousFigure?[p(e.quad,r),o.data.draw,i.data.draw]:[p(e.quad,r)],[n.general.canvasDimensions[0],n.general.canvasDimensions[1],n.general.showLissajousFigure]);return{shared_inputs:e,drawGradient:c,drawLissajousCurve:o,drawPoints:i,sampleDepthTex:t,outputTexture:x,renderTexture:r,gradientTexture:a}}function Z(n){const e={...u,time:n};return K(e)}const P=new N,Q=document.getElementById("mainCanvas"),w=Q.getContext("webgl2",{antialias:!1});if(!w)throw new Error("Unable to initialize WebGL2");const $=new S(w),m=new A($);P.subscribe((n,e)=>{const t=Object.fromEntries(n);m.submit(e,t),m.draw("outputTexture")});const H=new C;function W(n,e){const{addedAssets:t,deletedAssetIds:c,graphId:o}=H.update(Z(n));e&&E(t[o]),P.transaction(t,c,o)}const J=document.getElementById("inputRange");let z=parseFloat(J.value)/100;const nn=document.getElementById("saveFrameBtn");nn.addEventListener("click",()=>{const n=m.capture("outputTexture"),e=u.general.canvasDimensions[0],t=u.general.canvasDimensions[1],c=document.createElement("canvas");c.width=e,c.height=t;const o=c.getContext("2d");if(!o)throw new Error("Unable to create 2D context for image saving");const i=o.createImageData(e,t);i.data.set(n),o.putImageData(i,0,0);const a=document.createElement("a");a.download="frame.png",a.href=c.toDataURL(),a.click()});let d=0;function I(){z=d%300/300,W(z,d===0),d+=.5,requestAnimationFrame(I)}I();
